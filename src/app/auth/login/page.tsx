...
import { CircleDollarSign, Eye, EyeOff, Lock, Mail } from 'lucide-react'
...
export default function LoginPage() {
  ...
  const handleLogin = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!rememberMe) {
      ...
      sessionStorage.setItem('pdp_session_only', 'true')
    }
  ...
  return (
    <main className="grid min-h-screen place-items-center px-4 py-12">
      ...
      <div className="relative w-full max-w-md rounded-[2rem] border border-border bg-card/90 p-8 shadow-2xl backdrop-blur">
        <Link href="/" className="mb-8 inline-flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-muted-foreground">Live-looking tools, simulated data</div>
          </div>
        </Link>

        <h1 className="text-3xl font-bold text-white">Welcome back</h1>
        <p className="mt-2 text-sm text-muted-foreground">Sign in to your dashboard, bots, signals, and copy trading workspace.</p>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <label className="block">
            <div className="mb-2 flex items-center justify-between">
              ...
              <button type="button" onClick={handleReset} className="text-xs font-semibold text-primary hover:underline" disabled={resetting}>
          ...
          <Button type="submit" className="w-full justify-center rounded-2xl py-3" disabled={loading}>
